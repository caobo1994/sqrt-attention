...
No chunked fallback – each method stops at first OOM.
Fixed UnboundLocalError and improved error handling.
Results saved to full_comparison.json.
Usage: python full_benchmark.py
"""
...
def csa_attention(q, k, v, window_size=512, global_tokens=64):
    B, H, L, D = q.shape
    # Build mask: all positions attend to global tokens and local window
...
# ---------- Benchmark function (fixed variable deletion) ----------
def test_method(device, L, method, **kwargs):
    ...
    try:
        if method == 'dense':
            ...
            out, R = sqrt_block_attention(q, k, v)
        elif method == 'csa':
        ...
        # Safely delete tensors if they exist
        if q is not None:
            del q
        if k is not None:
            del k
    ...
    if method == 'dense':
        ...
        mem_mask = L * L / 1024**2   # bool mask (1 byte per element)
...
def main():
    ...
    # Lengths: 1K, 2K, 4K, ... up to 4M (4194304)
    lengths = [1024]
    ...
    # Define methods and their kwargs
    methods = {
    ...
    for L in lengths:
        ...
        for name, cfg in methods.items():
            if stop_flag[name]:
                print(f"  {name}: skipped (previous OOM)")
