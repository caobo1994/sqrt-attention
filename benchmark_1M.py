#!/usr/bin/env python3
"""
√Block Attention benchmark from 1K to 1M length on M2 Pro.
Only √Block is tested (Dense would OOM beyond 8K).
Results saved to long_benchmark.json and printed as table.
Usage: python benchmark_1m.py
"""
...
def chunked_scaled_dot_product_attention(query, key, value, chunk_size=2048):
...
def test_length(device, L, force_chunked=False):
    ...
    # Decide chunking: force if L is large or if we are beyond 64k
    ...
    # Estimate peak memory (rough: QKV + out + intermediates)
    # Actually memory usage is dominated by QKV (~3 * L * 64 * 2 bytes)
    mem_qkv = 3 * L * 64 * 2 / 1024**2
    # Representatives memory (R * 64 * 2)
    mem_repr = R * 64 * 2 / 1024**2 if R else 0
    # Add overhead
    est_mem = mem_qkv + mem_repr + 50  # 50MB overhead
...
def main():
    ...
    # Lengths: from 1K to 1M (geometric progression)
    lengths = []
    L = 1024
    while L <= 1048576:
        lengths.append(L)
        L = int(L * 1.5)  # increase by 50% each step
    # Ensure 1M is included
    if lengths[-1] != 1048576:
    ...
    for L in lengths:
        print(f"\n--- Length {L} ---")
        # For lengths very large, we can also try to skip if previous failed
